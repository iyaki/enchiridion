---
title: "Koa - web framework"
notion_id: 9ede282e-bf2e-4607-a380-4bb7ac0de08c
notion_url: https://app.notion.com/p/Koa-web-framework-9ede282ebf2e4607a3804bb7ac0de08c
last_edited: 2026-09-21T17:17:00.000Z
source_url: https://koajs.com/
tags: ["Framework/Library", "English", "Javascript", "Backend", "Untried"]
---
Koa
...
- app.asyncLocalStorage<Boolean|asyncLocalStorage> - pass true or an instance of AsyncLocalStorage to enable async local storage. See below for details. You can pass the settings to the constructor: const Koa = require('koa'); const app = new Koa({ proxy: true });or dynamically: const Koa = require('koa'); const app = new Koa(); app.proxy = true;
...
```plain text
app.keys = ['OEK5zjaAMPc3L6iK7PyUjCOziUH3rsrMKB9u8H07La1SkfwtuBoDnHaaPCkG5Brg', 'MNKeIebviQnCPo38ufHcSfw3FFv8EtnAe1xE02xkN1wkCV1B2z126U44yk2BQVK7']; app.keys = new KeyGrip(['OEK5zjaAMPc3L6iK7PyUjCOziUH3rsrMKB9u8H07La1SkfwtuBoDnHaaPCkG5Brg', 'MNKeIebviQnCPo38ufHcSfw3FFv8EtnAe1xE02xkN1wkCV1B2z126U44yk2BQVK7'], 'sha256');
```
...
```plain text
const app = new Koa({ asyncLocalStorage: true }) app.use(async (ctx, next) => { callSomeFunction() }) function callSomeFunction () { app.currentContext = {} /* ctx of the middleware above */ }
```

Starting in v3.1.0, you can also pass your own AsyncLocalStorage instance:

```plain text
const asyncLocalStorage = new AsyncLocalStorage() const app = new Koa({ asyncLocalStorage }) app.use(async (ctx, next) => { callSomeFunction() }) function callSomeFunction () { const ctx = asyncLocalStorage.getStore() }
```
...
```plain text
// freshness check requires status 20x or 304 ctx.status = 200; ctx.set('ETag', '123'); // cache is ok if (ctx.fresh) { ctx.status = 304; return; } // cache is stale // fetch new data ctx.body = await db.find('something');
```
...
```plain text
const app = new Koa({ proxy: true, maxIpsCount: 1, // only one proxy in front of the server }); // request.header['X-Forwarded-For'] === [ '127.0.0.1', '127.0.0.2' ]; // ctx.ips === [ '127.0.0.2' ];
```
...
```plain text
// With Content-Type: text/html; charset=utf-8 ctx.is('html'); // => 'html' ctx.is('text/html'); // => 'text/html' ctx.is('text/*', 'text/html'); // => 'text/html' // When Content-Type is application/json ctx.is('json', 'urlencoded'); // => 'json' ctx.is('application/json'); // => 'application/json' ctx.is('html', 'application/*'); // => 'application/json' ctx.is('html'); // => false
```
...
```plain text
ctx.response.status = 200; // Or whatever other status ctx.response.status = 204;
```
...
```plain text
// This must be always set first before status, since null | undefined // body automatically sets the status to 204 ctx.body = null; // Now we override the 204 status with the desired one ctx.status = 200;
```
...
```plain text
app.use(async (ctx, next) => { await next() ctx.assert.equal('object', typeof ctx.body, 500, 'some dev did something wrong') })
```
...
```plain text
ctx.redirect('/login'); ctx.redirect('http://google.com');
```
...
Flush any set headers to the client. This is a proxy for res.flushHeaders()
