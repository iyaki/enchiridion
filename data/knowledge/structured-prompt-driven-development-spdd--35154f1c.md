...
The goal of the SPDD workflow is to turn business input → abstraction → execution → validation → release into a _“closed loop”_1—and to make sure prompt assets and code evolve together, not separately.
...
1. 1. **Base Validations (Regression & New)** **Given** an invalid request (e.g., missing `modelId`, negative tokens) **When** backend validates request **Then** return HTTP 400 with appropriate error messages.
2. 2. **Standard Plan with Model-Aware Overage** **Given** a “Standard” customer with a 100,000 monthly quota, 90,000 used so far. Overage for “fast-model” is `$0.01/1K`. **When** submitting 30,000 tokens for “fast-model” **Then** bill shows: 10,000 from quota, 20,000 overage, $0.20 charge.
3. 3. **Premium Plan with Split Rates** **Given** a “Premium” customer. For “reasoning-model”, Prompt is `$0.03/1K`, Completion is `$0.06/1K`. **When** submitting 10,000 prompt and 20,000 completion tokens for “reasoning-model” **Then** bill shows: 0 from quota, `$0.30` prompt charge, `$1.20` completion charge, total `$1.50`.
