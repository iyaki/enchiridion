---
title: "cheat.sh - Terminal CheatSheets"
notion_id: 10e22c8c-c852-4a11-b111-9a9e2a5c3e2e
notion_url: https://app.notion.com/p/cheat-sh-Terminal-CheatSheets-10e22c8cc8524a11b1119a9e2a5c3e2e
last_edited: 2022-12-21T14:16:00.000Z
source_url: https://github.com/chubin/cheat.sh
tags: ["English", "Linux", "Untried", "Tool", "CheatSheet"]
---
...
- Features
- Usage
- Command line client, cht.sh Installation Client usage Tab-completion Bash Tab completion ZSH Tab completion Stealth mode Windows command line client
- Self-Hosting Docker
- Editors integration Vim Emacs Visual Studio Code Sublime IntelliJ IDEA QT Creator
- Special pages
- Search
- Programming languages cheat sheets
- Cheat sheets sources
- How to contribute How to edit a cheat sheet How to add a cheat sheet How to add a cheat sheet repository
...
```plain text
curl cht.sh/go/Pointers curl cht.sh/scala/Functions curl cht.sh/python/lambda
```
...
```plain text
curl cht.sh/go/reverse+a+list curl cht.sh/python/random+list+elements curl cht.sh/js/parse+json curl cht.sh/lua/merge+tables curl cht.sh/clojure/variadic+function
```
...
```plain text
curl cht.sh/python/random+string curl cht.sh/python/random+string/1 curl cht.sh/python/random+string/2
```
...
```plain text
curl cht.sh/go/reverse+a+list\?Q curl cht.sh/python/random+list+elements\?Q curl cht.sh/js/parse+json\?Q curl cht.sh/lua/merge+tables\?QT curl cht.sh/clojure/variadic+function\?QT
```
...
```plain text
$ cht.sh --shell cht.sh> cd go cht.sh/go> reverse a list
```

or even start the client in this context:

```plain text
$ cht.sh --shell go cht.sh/go> reverse a list ... cht.sh/go> join a list ...
```
...
```plain text
$ cht.sh --shell go ... cht.sh/go> /python dictionary comprehension ...
```

If you want to copy the last answer into the clipboard, you can use the c (copy) command, or C (ccopy, without comments).

```plain text
cht.sh/python> append file # python - How do you append to a file? with open("test.txt", "a") as myfile: myfile.write("appended text") cht.sh/python> C copy: 2 lines copied to the selection
```
...
```plain text
CHTSH_CURL_OPTIONS="-A curl" # curl options used for cht.sh queries CHTSH_URL=https://cht.sh # URL of the cheat.sh server
```
...
Feature Emacs Sublime Vim VSCode IDEA QtCreator Command queries ✓ ✓ ✓ ✓ ✓ ✓ Queries from buffer ✓ ✓ ✓ Toggle comments ✓ ✓ ✓ ✓ Prev/next answer ✓ ✓ ✓ ✓ Multiple answers ✓ ✓ Warnings as queries ✓ Queries history ✓ ✓ Session id ✓ Configurable server ✓ ✓ ✓ ✓
...
```plain text
:help description of all special pages and options :intro cheat.sh introduction, covering the most important usage questions :list list all cheat sheets (can be used in a subsection too: /go/:list)
```

Command line client cht.sh and shells support:

```plain text
:cht.sh code of the cht.sh client :bash_completion bash function for tab completion :bash bash function and tab completion setup :fish fish function and tab completion setup :zsh zsh function and tab completion setup
```

Editors support:

```plain text
:vim cheat.sh support for Vim :emacs cheat.sh function for Emacs :emacs-ivy cheat.sh function for Emacs (uses ivy)
```

Other pages:

```plain text
:post how to post new cheat sheet :styles list of color styles :styles-demo show color styles usage examples :random fetches a random page (can be used in a subsection too: /go/:random)
```
...
```plain text
/~snapshot look for snapshot in the first level cheat sheets /scala/~currying look for currying in scala cheat sheets
```
...
```plain text
i case insensitive search b word boundaries r recursive search
```
...
```plain text
lua lua/hello lua/:list lua/:learn
```
...
Prefix Topic Basics StackOverflow cmake/ CMake ✓ ✓ django/ Django ✓ flask/ Flask ✓ git/ Git ✓ ✓
...
Cheat sheets Repository C/U* Stars Creation Date UNIX/Linux, programming cheat.sheets May 1, 2017 UNIX/Linux commands tldr-pages/tldr Dec 8, 2013 UNIX/Linux commands cheat/cheat Jul 28, 2013 Programming languages adambard/learnxinyminutes-docs Jun 23, 2013 Go a8m/go-lang-cheat-sheet Feb 9, 2014 Perl pkrumnis/perl1line.txt Nov 4, 2011 Programming languages StackOverflow 14M N/A Sep 15, 2008
