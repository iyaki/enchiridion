...
- Prints file, line number, v
```

```plain text
iolation, and suggestion
...
That’s the manual estimate. The [setup prompt](https://hvpandya.com/llm-design-systems/#the-setup) gets an AI agent through all six steps in one session. You review its output, adjust token values to your taste, and commit. Typical turnaround is a few hours.
