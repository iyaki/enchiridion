...
notion_id: 680b50a0-8e31-4b7e-b5ff-642c61929ec1
notion_url: https://app.notion.com/p/DuckDB-An-in-process-SQL-OLAP-database-management-system-680b50a08e314b7eb5ff642c61929ec1
last_edited: 2023-01-18T14:02:00.000Z
...
[continue reading](https://duckdb.org/2022/11/25/duckcon.html)
...
[continue reading](https://duckdb.org/2022/11/14/announcing-duckdb-060.html)
...
[continue reading](https://duckdb.org/2022/10/28/lightweight-compression.html)
...
**FSST**. The [Fast Static Symbol Table](https://github.com/duckdb/duckdb/pull/4366) compression algorithm is introduced in this version. This state-of-the-art compression algorithm compresses data _inside_ strings using a dictionary, while maintaining support for efficient scans and random look-ups. This greatly increases the compression ratio of strings that have many unique values but with common elements, such as e-mail addresses or URLs.
...
**COLUMNS Expression**. This release adds support for [the ](https://github.com/duckdb/duckdb/pull/5120)[`COLUMNS`](https://github.com/duckdb/duckdb/pull/5120)[ expression](https://github.com/duckdb/duckdb/pull/5120), inspired by [the Clickhouse syntax](https://clickhouse.com/docs/en/sql-reference/statements/select/#columns-expression). The `COLUMNS` expression allows you to execute expressions or functions on multiple columns without having to duplicate the full expression.
...
The `COLUMNS` expression supports all star expressions, including [the ](https://duckdb.org/docs/sql/query_syntax/select)[`EXCLUDE`](https://duckdb.org/docs/sql/query_syntax/select)[ and ](https://duckdb.org/docs/sql/query_syntax/select)[`REPLACE`](https://duckdb.org/docs/sql/query_syntax/select)[ syntax](https://duckdb.org/docs/sql/query_syntax/select). In addition, the `COLUMNS` expression can take a regular expression as parameter:
...
**DuckBox Rendering**. This release includes a [new ](https://github.com/duckdb/duckdb/pull/5140)[`.mode duckbox`](https://github.com/duckdb/duckdb/pull/5140)[ rendering](https://github.com/duckdb/duckdb/pull/5140) that is used by default. This box rendering adapts to the size of the shell, and leaves out columns and rows to provide a better overview of a result. It very quickly renders large result sets by leaving out rows in the middle. That way, typing `SELECT * FROM tbl` in the shell no longer blows it up. In fact, this can now be used to quickly get a good feel of a dataset instead.
...
[back to news archive](https://duckdb.org/news/)
