---
title: "So You Received a Security Report. Now What? — The PHP Foundation — Supporting, Advancing, and Developing the PHP Language"
notion_id: 3df54f1c-7d23-8113-b02f-c5b36825ed99
notion_url: https://app.notion.com/p/So-You-Received-a-Security-Report-Now-What-The-PHP-Foundation-Supporting-Advancing-and-Devel-3df54f1c7d238113b02fc5b36825ed99
last_edited: 2026-09-18T01:18:00.000Z
source_url: https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/
tags: ["Guide", "thePHP.cc - The PHP Consulting Company", "English", "Security", "PHP", "Software Development", "Open Source", "Documentation"]
---
...
- **If you only have five minutes right now,** read the [Cheatsheet](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#cheatsheet) and
section [1](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#1.-do-not-panic-and-do-not-go-public).
...
released. → [§1](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#1.-do-not-panic-and-do-not-go-public)
2. **Acknowledge the report** within a few days, even before you have assessed it.
→ [§2](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#2.-acknowledge-the-report)
3. **Triage:** understand the claim, then decide: valid, invalid, or out of scope.
→ [§3](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#3.-triage-the-report)
4. **Reproduce safely.** Never run proof-of-concept code on your own machine; use an
isolated environment. → [§4](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#4.-handle-proof-of-concept-code-safely)
5. **Fix privately.** Use a temporary private fork; do not push to any public branch.
→ [§5](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#5.-prepare-the-fix-privately)
...
→ [§6](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#6.-write-the-github-security-advisory)
7. **Publish in the right order:** merge → tag & release → publish the advisory →
submit it to FriendsOfPHP → announce.
→ [§7](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#7.-coordinate-the-release-and-publication)
8. **Wrap up:** credit the reporter, check older branches, do a short retrospective.
→ [§8](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#8.-wrap-up)
9. **Improve your posture** so the next report is easier: SECURITY.md, private
vulnerability reporting, 2FA, hardened CI. → [§9](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#9.-improving-your-security-posture-for-the-long-term)
10. **Ask for help** whenever you are unsure. → [§10](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#10.-getting-help)

---

## 1. Do not panic and do not go public

_You are here if: a report just landed and your pulse went up._
...
injection in login handler" _is_ the disclosure.
...
wrong. Part of your job in the next steps is to find out which, and _you_ make that
...
_You are here if: you have read the report once and have not replied yet._
...
enabled (see [§9](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#9.-improving-your-security-posture-for-the-long-term)), this is a
...
_You are here if: you have acknowledged the report and now need to decide what it is._
...
_You are here if: the report includes a script, a payload, a crafted file, or
step-by-step exploitation instructions._
...
And remember what a PoC _is_: a program written by a stranger, designed to break
...
clipboard sharing disabled, and a snapshot taken _before_ you run anything so you can
...
_how_ it triggers the bug is exactly the insight you need for the fix and for the
regression test. In fact, this is the natural moment to turn the reproducer into a
failing test ([§5](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#5.-prepare-the-fix-privately)). Many maintainers treat "I have a
...
_You are here if: the issue is confirmed and you are ready to write code._

GitHub Security Advisories give you a **temporary private fork** for exactly this
purpose. On the advisory page, click _"Start a temporary private fork"_. Then:
...
([§4](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#4.-handle-proof-of-concept-code-safely)), you already have most of it;
distill it into a safe, minimal test case that demonstrates the _condition_ rather
...
_You are here if: the fix exists and you are filling in the advisory form._
...
focuses on the part the walkthrough cannot do for you: what to put _into_ the fields.
...
`>= 10.0.0, < 10.5.17`, and one _Affected product_ entry per affected release
...
it used to, because Composer no longer merely _reports_ your advisory. It
_enforces_ it. Read on.
...
unified _dependency policy_ framework that also covers packages flagged as malware
...
the strongest argument against fixing silently ([§5](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#5.-prepare-the-fix-privately)):
...
Composer (see step 4 in [§7](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#7.-coordinate-the-release-and-publication) for what
...
builds, and their bug reports will land in _your_ inbox. This is not hypothetical:
...
advisories from the GitHub Advisory Database _and_ from
...
([§7](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#7.-coordinate-the-release-and-publication)) and the fastest way to fix what
...
The flip side is worth telling your _users_ about, too (in your announcement or your
...
- **Description:** enough for a user to understand _whether they are affected_ and
_what to do_ (upgrade to which version; workaround if one exists). Aim for the
...
something that arrives when it arrives: request it once you have _accepted_ the
...
_"I have an existing CVE ID"_ instead of requesting a second one.
...
_You are here if: fix ready, advisory drafted, everyone in the private thread agrees._
...
Packagist, which consumes the _reviewed_ GitHub Advisory Database, and GitHub's
...
_You are here if: the advisory is published and the adrenaline is fading._
...
([§7](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#7.-coordinate-the-release-and-publication)) is there to short-circuit. If it
still has not arrived after a few days, check the ecosystem and package-name fields
first ([§6](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#6.-write-the-github-security-advisory)); those are the usual suspects
when matching silently fails.
- **Verify the blocking is not too eager:** in a scratch project, check that the
_fixed_ version and unaffected release series still install and update cleanly.
...
correction path is described in [§6](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#composer-blocks-the-versions-your-advisory-lists).
...
([§5](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#5.-prepare-the-fix-privately)); what you are looking for here is the one
...
_You are here if: the storm has passed and you want the next one to be smaller. This
...
a notification: _security advisories_, which is where a report about **your** code
arrives, and _Dependabot alerts_, which tell you that a dependency **you** use has
...
1. **Strong 2FA/MFA everywhere that can publish:** GitHub, Packagist, your email
...
2. **Protect your release path:** branch protection on `main`, tag protection for
release tags, and no long-lived personal access tokens with broad scopes lying
around in CI secrets or on disk.
3. **Never re-tag a released version.** If a release has a problem, ship a new
...
1. **Delete what does not need to exist.** Every branch in your repository is a door:
...
1. **Treat your workflow files as security-relevant code.** They run with
...
2. **Run ****`composer audit`**** in CI** so you learn about vulnerable dependencies from
...
_your_ dependencies too ([§6](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#composer-blocks-the-versions-your-advisory-lists)).
...
1. **Keep an isolated reproduction environment ready** ([§4](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#4.-handle-proof-of-concept-code-safely)),
so that "spin up the VM" is a two-minute routine rather than a reason to take the
dangerous shortcut.
2. **Treat security reports as a normal category of work,** with the small process
from this guide, rather than as emergencies. The calmer the routine, the better
every individual case goes.
3. **Know who to call.** Which brings us to the final section.
...
- **Background on the team:**[Announcing the Ecosystem Security Team](https://thephp.foundation/blog/2026/05/18/announcing-ecosystem-security-team/) ·
...
- [Composer: ](https://getcomposer.org/doc/03-cli.md#audit)[`composer audit`](https://getcomposer.org/doc/03-cli.md#audit) and the
...
weaknesses behind [§9](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#9.-improving-your-security-posture-for-the-long-term)
...
_If anything here was unclear, took too long to figure out, or turned out to be wrong
in practice, please tell us; that feedback directly improves the experience of the
next maintainer. You can reach us at the addresses in _[_§10_](https://thephp.foundation/blog/2026/08/19/so-you-received-a-security-report-now-what/#10.-getting-help)_._
