...
notion_id: dfd15f27-aef1-4848-a6f3-9687c3534695
notion_url: https://app.notion.com/p/Our-top-Core-Web-Vitals-recommendations-for-2023-dfd15f27aef14848a6f39687c3534695
last_edited: 2023-01-17T11:28:00.000Z
source_url: https://web.dev/top-cwv-2023/
tags: ["Web Development", "Article", "English"]
---
...
On the Chrome team, we've spent the last year trying to answer this question: _what are the most important recommendations we can give to developers to help them improve performance for their users?_
...
## Largest Contentful Paint (LCP) [#](https://web.dev/top-cwv-2023/#largest-contentful-paint-lcp)
...
### Ensure the LCP resource is discoverable from the HTML source [#](https://web.dev/top-cwv-2023/#ensure-the-lcp-resource-is-discoverable-from-the-html-source)
...
What may not be obvious to many developers is that the time it takes to load an image is just one part of the challenge. Another critical part is the time _before_ an image starts loading, and HTTP Archive data suggests that's actually where many sites get tripped up.
...
### Ensure the LCP resource is prioritized [#](https://web.dev/top-cwv-2023/#ensure-the-lcp-resource-is-prioritized)
...
- **Defer non-critical resources when possible.** Either by moving them to the end of your document, using native lazy-loading for [images](https://web.dev/browser-level-image-lazy-loading/) or [iframes](https://web.dev/iframe-lazy-loading/), or loading them asynchronously via JavaScript.

### Use a CDN to optimize document and resource TTFB [#](https://web.dev/top-cwv-2023/#use-a-cdn-to-optimize-document-and-resource-ttfb)
...
In general, any time you can serve content directly from the edge (avoiding a trip to your origin server) it's a performance win. And even in cases where you _do_ have to make the journey all the way back to your origin server, CDNs are generally optimized to do that much more quickly, so it's a win either way.

## Cumulative Layout Shift (CLS) [#](https://web.dev/top-cwv-2023/#cumulative-layout-shift-cls)
...
### Ensure pages are eligible for bfcache [#](https://web.dev/top-cwv-2023/#ensure-pages-are-eligible-for-bfcache)
...
### Avoid animations/transitions that use layout-inducing CSS properties [#](https://web.dev/top-cwv-2023/#avoid-animationstransitions-that-use-layout-inducing-css-properties)
...
While HTTP Archive data can't conclusively connect animations to layout shifts, the data does show that pages that animate any CSS property that _could_ affect layout are 15% less likely to have "good" CLS than pages overall. Some properties are associated with worse CLS than others. For instance, pages that animate `margin` or `border` widths have "poor" CLS at almost twice the rate that pages overall are assessed as poor.

This is perhaps not surprising, because any time you transition or animate _any_ layout-inducing CSS property, it will result in [layout shifts](https://web.dev/cls/#layout-shifts-in-detail), and if those layout shifts are not within 500 milliseconds of a user interaction, they will impact CLS.
...
Preferring animation of CSS properties that can be updated on the browser's compositor thread has long been [a performance best practice](https://web.dev/animations-guide/) because it moves that work onto the GPU and off the main thread. And in addition to it being a general performance best practice, it can also help improve CLS.

As a general rule, never animate or transition any CSS property that requires the browser to update the page layout, unless you're doing it in response to a user tap or key press (though [not ](https://web.dev/cls/#user-initiated-layout-shifts)[`hover`](https://web.dev/cls/#user-initiated-layout-shifts)). And whenever possible, prefer transitions and animations using the CSS [`transform`](https://developer.mozilla.org/docs/Web/CSS/transform) property.
...
## First Input Delay (FID) [#](https://web.dev/top-cwv-2023/#first-input-delay-fid)
...
### Avoid or break up long tasks [#](https://web.dev/top-cwv-2023/#avoid-or-break-up-long-tasks)
...
Another option is to consider using APIs such as [`isInputPending`](https://web.dev/optimize-long-tasks/#yield-only-when-necessary) and the [Scheduler API](https://web.dev/optimize-long-tasks/#a-dedicated-scheduler-api). `isInputPending` is a function that returns a boolean value that indicates whether a user input is pending. If it returns `true`, you can yield to the main thread so it can handle the pending user input.
...
### Avoid unnecessary JavaScript [#](https://web.dev/top-cwv-2023/#avoid-unnecessary-javascript)
...
### Avoid large rendering updates [#](https://web.dev/top-cwv-2023/#avoid-large-rendering-updates)
...
## Conclusion [#](https://web.dev/top-cwv-2023/#conclusion)
