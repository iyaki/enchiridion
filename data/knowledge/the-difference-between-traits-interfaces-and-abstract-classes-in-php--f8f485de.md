...
notion_id: f8f485de-20a4-49b5-b4d0-c1237bca09f7
notion_url: https://app.notion.com/p/The-difference-between-Traits-Interfaces-and-Abstract-Classes-in-PHP-f8f485de20a449b5b4d0c1237bca09f7
last_edited: 2023-04-25T13:22:00.000Z
source_url: https://aschmelyun.com/blog/the-difference-between-traits-interfaces-and-abstract-classes-in-php/
tags: ["Andrew Schmelyun Blog", "English", "PHP", "Article"]
---
[https://aschmelyun.com/blog/the-difference-between-traits-interfaces-and-abstract-classes-in-php/](https://aschmelyun.com/blog/the-difference-between-traits-interfaces-and-abstract-classes-in-php/)
...
```plain text
<?phpabstract class Pet{ abstract protected function greet(); public function hasFur() { return true; }}
```
...
```plain text
<?phpclass Cat extends Pet{ public function greet() { return 'Meow!'; }}$cat = new Cat();$cat->greet(); // Meow!$cat->hasFur(); // true
```
...
```plain text
<?phptrait HasLegs{ public function walk() { $steps = 0; while (true) { $steps++; } }}
```

```plain text
<?phptrait HasFins{ public function swim() { $laps = 0; while (true) { $laps++; } }}
```
...
```plain text
<?phpclass Cat{ use HasLegs;}$cat = new Cat();$cat->walk();
```
...
```plain text
<?phpclass Duck{ use HasLegs, HasFins;}$duck = new Duck();$duck->walk();//or$duck->swim();
```
...
```plain text
<?phpinterface Bug{ public function legs(); public function eyes();}
```
...
```plain text
<?phpclass Spider implements Bug{ public function legs() { return 8; } public function eyes() { return 8; }}
```

```plain text
<?phpclass Beetle implements Bug{ public function legs() { return 6; } public function eyes() { return 2; }}
```
...
Subscribe using the form below and about 2-3 times a month you'll receive an email containing project updates, video previews, and inspiring links I've found across the web dev and tech space.
