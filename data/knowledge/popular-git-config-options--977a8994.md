...
notion_id: 977a8994-f78e-4379-b7d2-5fd0c0f9ca9f
notion_url: https://app.notion.com/p/Popular-git-config-options-977a8994f78e4379b7d25fd0c0f9ca9f
last_edited: 2024-02-21T00:16:00.000Z
source_url: https://jvns.ca/blog/2024/02/16/popular-git-config-options/
tags: ["Julia Evans", "English", "Programming", "Productivity", "Article", "CheatSheet"]
---
...
These [`push.default`](https://git-scm.com/docs/git-config#Documentation/git-config.txt-pushdefault) options tell `git push` to automatically push the current branch to a remote branch with the same name.
...
what are your favourite git config options to set? Right now I only really have `git config push.autosetupremote true` and `git config
init.defaultBranch main` set in my `~/.gitconfig`, curious about what other people set
...
- [and more](https://jvns.ca/blog/2024/02/16/popular-git-config-options/#and-more)
- [how to set these](https://jvns.ca/blog/2024/02/16/popular-git-config-options/#how-to-set-these)
- [changes I’ve made after writing this post](https://jvns.ca/blog/2024/02/16/popular-git-config-options/#changes-i-ve-made-after-writing-this-post)
...
`core.excludeFiles = ~/.gitignore` lets you have a global gitignore file that applies to all repositories, for things like `.idea` or `.DS_Store` that you never want to commit to any repo.
...
I’ve learned to be cautious about setting new config options – it takes me a long time to get used to the new behaviour and if I change too many things at once I just get confused. `branch.sort -committerdate` is something I was already using anyway (through an alias), and I’m pretty sold that `diff.algorithm
