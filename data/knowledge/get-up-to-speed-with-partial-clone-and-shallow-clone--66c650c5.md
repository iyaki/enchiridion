...
tags: ["Programming", "Productivity", "Article", "English"]
---
...
Git’s partial clone feature is enabled by specifying [the ](https://git-scm.com/docs/git-clone#Documentation/git-clone.txt---filterltfilter-specgt)[`--filter`](https://git-scm.com/docs/git-clone#Documentation/git-clone.txt---filterltfilter-specgt)[ option in your ](https://git-scm.com/docs/git-clone#Documentation/git-clone.txt---filterltfilter-specgt)[`git clone`](https://git-scm.com/docs/git-clone#Documentation/git-clone.txt---filterltfilter-specgt)[ command](https://git-scm.com/docs/git-clone#Documentation/git-clone.txt---filterltfilter-specgt). The full list of filter options exist in [the ](https://git-scm.com/docs/git-rev-list#Documentation/git-rev-list.txt---filterltfilter-specgt)[`git rev-list`](https://git-scm.com/docs/git-rev-list#Documentation/git-rev-list.txt---filterltfilter-specgt)[ documentation](https://git-scm.com/docs/git-rev-list#Documentation/git-rev-list.txt---filterltfilter-specgt), since you can use `git rev-list --filter=<filter> --all` to see which objects in your repository match the filter. There are several filters available, but the server can choose to deny your filter and revert to a full clone.
...
**Warning:** While writing this article, we were putting treeless clones to the test beyond the typical limits. We noticed that repositories that contain _submodules_ behave very poorly with treeless clones. Specifically, if you run `git fetch` in a treeless clone, then the logic in Git that looks for changed submodules will trigger a tree request for every new commit! This behavior can be avoided by running `git config fetch.recurseSubmodules false` in your treeless clones. We are working on a more robust fix in the Git client.

## Shallow clones

Partial clones are relatively new to Git, but there is an older feature that does something very similar to a treeless clone: shallow clones. Shallow clones use [the ](https://git-scm.com/docs/git-clone#Documentation/git-clone.txt---depthltdepthgt)[`--depth=<N>`](https://git-scm.com/docs/git-clone#Documentation/git-clone.txt---depthltdepthgt)[ parameter in ](https://git-scm.com/docs/git-clone#Documentation/git-clone.txt---depthltdepthgt)[`git clone`](https://git-scm.com/docs/git-clone#Documentation/git-clone.txt---depthltdepthgt) to truncate the commit history. Typically, `--depth=1` signifies that we only care about the most recent commits. Shallow clones are best combined with the `--single-branch --branch=<branch>` options as well, to ensure we only download the data for the commit we plan to use immediately.
...
Fellow GitHub engineer [**@solmazabbaspour**](https://github.com/solmazabbaspour) designed and ran an experiment to compare these different clone options on a variety of open source repositories. She will post a blog post tomorrow giving full details and data for the experiment, but I’ll share the executive summary here. Here are some common themes we identified that could help you choose the right scenario for your own usage:
